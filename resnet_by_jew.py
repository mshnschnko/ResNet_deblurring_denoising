...
Original file is located at
    https://colab.research.google.com/drive/1jZ63UhupxL-s4VDWYxPau2rJ0-E-FvI5
"""
...
def images_for_denoising():
    ...
    return list_images(relpath('drive/MyDrive/ResNet datasets/image_dataset/circles2'), True)
...
# Train model
    model.compile(optimizer= Adam(beta_2 = 0.9),loss='mean_squared_error') #mean_squared_error
...
if __name__ == "__main__":
    # imgs = list_images(relpath('drive/MyDrive/ResNet datasets/bead_folder/'), True)
    img = read_image("drive/MyDrive/ResNet datasets/low_res/test36T.tiff", 1)
    # model, hist = learn_denoising_model(quick_mode = False)
    # model.save('drive/MyDrive/ResNet datasets/denoising_model_10_epo_ideal_training')
    model = keras.models.load_model('drive/MyDrive/ResNet datasets/denoising_model_10_epo_ideal_training')
    
    res_img = restore_image(img, model)
    preprocessing.image.save_img(f"TEST.tif", np.reshape(res_img, [36, 36, 1]))

    # for i in range(len(imgs)):
    #   img = read_image(imgs[i], 1)
    #   res_img = restore_image(img, model)
    #   preprocessing.image.save_img(f"bead_0{str(i)}_dnr.tif", np.reshape(res_img, [36, 36, 1]))
    # res_visual(hist)
